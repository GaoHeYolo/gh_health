...
public class OrderController {
    ...
    public Result submit(@RequestBody Map map) {
        ...
        //从Redis中获取保存的验证码
        String validateCodeInRedis = jedisPool.getResource().get(emailAddress + RedisMessageConstant.SENDTYPE_ORDER);
        ...
        if (validateCodeInRedis != null && validateCode != null && validateCode.equals(validateCodeInRedis)) {
            ...
            if (result.isFlag()) {
                //预约成功，可以为用户发送邮件
                try {
                    MailUtils.sendMail(MailUtils.ORDER_NOTICE, emailAddress, (String) map.get("orderDate"));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
