...
import com.gh.dao.OrderSettingDao;
...
public class OrderServiceImpl implements OrderService {
    ...
    public Result order(Map map) throws Exception{
        //1.用户所选择的日期是否已经提前进行了预约设置，如果没有设置则无法进行预约
        ...
        String  emailAddress = (String) map.get("emailAddress");//获取用户页面输入的手机号
        Member member = memberDao.findByEmailAddress(emailAddress);
        if (member!=null){
            //判断是否在重复预约
            ...
            member.setEmail((String) map.get("emailAddress"));
